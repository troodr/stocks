...
Identify the top performing assets from the folloing files:  
1. [Stock Prices](https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2023/2023-02-07/big_tech_stock_prices.csv).  
2. [Company Names](https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2023/2023-02-07/big_tech_companies.csv).

## Code

The `code` requires **daily_data.csv** file which contains __ohclv__:
...
#### Join the aggregate data to stock table
```{r}
#| include: true
#| warning: false
#| message: false
...
base_url = "https://raw.githubusercontent.com/rahul-c1/TidyTuesday/master/2023/W6/logos/"
...
df_sp %>% 
  group_by(stock_symbol) %>% 
  summarise(avg_price = mean(adj_close,na.rm=TRUE))

```

```{r, warning=FALSE,fig.height=3,fig.width=9,fig.cap="Average Price for each symbol"}
...
library(patchwork)
library(hrbrthemse) # remotes::install_git("https://codeberg.org/hrbrmstr/hrbrthemes.git")
p1+p2+p3
```
```{r}
#| include: false
